...
module.exports = class extends Base {
  ...
  getOrdersAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const userId = _this6.post('userId');
      const orderStatus = _this6.post('orderStatus');
      const pageSize = _this6.post('pageSize');
      const page = _this6.post('page');
      let count = 0;
      let orderData = [];
      let order = [];
      if (orderStatus == 8) {
        count = _this6.model('order').where({ user_id: userId }).select();
        orderData = yield _this6.model('order').where({ user_id: userId }).page(page, pageSize).select(); // join({
        for (var i = 0; i < orderData.length; i++) {
          order[i] = {};
          order[i].status = orderData[i].order_status;
          order[i].amount = orderData[i].order_price;
          if (orderData[i].order_status == 1) {
            orderData[i] = yield _this6.model('order_goods').where('nideshop_order_goods.order_id = ' + orderData[i].order_sn).select();
            continue;
          }
          orderData[i] = yield _this6.model('order_goods').where('nideshop_order_goods.order_id = ' + orderData[i].order_sn).join({
            order_express: {
              join: 'left',
        ...
        count = _this6.model('order').where({ user_id: userId, order_status: orderStatus }).select();
        //orderData = await this.model('order').where({user_id:userId,order_status:orderStatus}).page(page,pageSize).select()

        // for(var i=0; i<orderData.length; i++) {
        //   order[i] = {}
        //   order[i].status = orderStatus
        //   order[i].amount = orderData[i].order_price
        //   //orderData[i] = await this.model('order_goods').where('nideshop_order_goods.order_id = '+ orderData[i].order_sn).select()
        //   //break;
        // }
      } else {
        count = _this6.model('order').where({ user_id: userId, order_status: orderStatus }).select();
        orderData = yield _this6.model('order').where({ user_id: userId, order_status: orderStatus }).page(page, pageSize).select();
        for (var i = 0; i < orderData.length; i++) {
          order[i] = {};
          order[i].status = orderStatus;
          order[i].amount = orderData[i].order_price;
      ...
      return _this6.success({ detail: orderData, order: order, count: count.length });
    })();
  }
};

...
module.exports = class extends Base {
  ...
  detailAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const addressInfo = yield _this2.model('address').where({ user_id: id }).select();
      if (!think.isEmpty(addressInfo)) {
        for (let i = 0; i < addressInfo.length; i++) {
          addressInfo[i].province_name = yield _this2.model('region').getRegionName(addressInfo[i].province_id);
          addressInfo[i].city_name = yield _this2.model('region').getRegionName(addressInfo[i].city_id);
          addressInfo[i].district_name = yield _this2.model('region').getRegionName(addressInfo[i].district_id);
  ...
  saveAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (think.isEmpty(addressId)) {
        addressData.id = (yield _this3.model('address').max('id')) + 1;
        addressId = addressData.id;
        yield _this3.model('address').add(addressData);
      } else {
      ...
      if (!think.isEmpty(addressInfo)) {
        addressInfo.province_name = yield _this3.model('region').getRegionName(addressInfo.province_id);
        addressInfo.city_name = yield _this3.model('region').getRegionName(addressInfo.city_id);
        addressInfo.district_name = yield _this3.model('region').getRegionName(addressInfo.district_id);

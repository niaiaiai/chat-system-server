var _form = require('alipay-sdk/lib/form');

var _form2 = _interopRequireDefault(_form);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _asyncToGenerator(fn) { return function () { var gen = fn.apply(this, arguments); return new Promise(function (resolve, reject) { function step(key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { return Promise.resolve(value).then(function (value) { step("next", value); }, function (err) { step("throw", err); }); } } return step("next"); }); }; }

const Base = require('./base.js');
const fs = require('fs');
const axios = require('axios');
var crypto = require('crypto');

const AlipaySdk = require('alipay-sdk').default;

const formData = new _form2.default();
formData.setMethod('get');

const alipaySdk = new AlipaySdk({
  gateway: 'https://openapi.alipaydev.com/gateway.do',
  appId: '2016092000553852',
  privateKey: '',
  format: 'json',
  charset: 'utf-8',
  alipayPublicKey: '',
  sign_type: 'RSA2'
});

module.exports = class extends Base {
  payAction() {
    var _this = this;

    return _asyncToGenerator(function* () {
      const amount = _this.post('amount');
      const subject = _this.post('subject');

      let date = new Date();
      var datestr = 'yyyy-MM-dd hh:mm:ss';
      var o = {
        "M+": date.getMonth() + 1, //月份   
        "d+": date.getDate(), //日   
        "h+": date.getHours(), //小时   
        "m+": date.getMinutes(), //分   
        "s+": date.getSeconds() //秒   
      };
      if (/(y+)/.test(datestr)) datestr = datestr.replace(RegExp.$1, (date.getFullYear() + "").substr(4 - RegExp.$1.length));
      for (var k in o) if (new RegExp("(" + k + ")").test(datestr)) datestr = datestr.replace(RegExp.$1, RegExp.$1.length == 1 ? o[k] : ("00" + o[k]).substr(("" + o[k]).length));

      let th = _this;

      formData.addField('returnUrl', 'http://192.168.10.165:9080/#/return');
      formData.addField('notifyUrl', 'http://47.107.155.139:8360/api/alipay/payParams');
      formData.addField('bizContent', {
        outTradeNo: date.getTime().toString(),
        productCode: 'FAST_INSTANT_TRADE_PAY',
        totalAmount: amount,
        subject: subject
      });

      try {
        const result = yield alipaySdk.exec('alipay.trade.page.pay', {}, { validateSign: true, formData: formData });

        return _this.success(result);
      } catch (err) {
        return _this.success(err);
      }

      // return this.success(result)
    })();
  }

  payParamsAction() {
    var _this2 = this;

    return _asyncToGenerator(function* () {
      const trade_status = _this2.post('trade_status');
      if (!trade_status) yield _this2.module('order').where({ id: trade_status }).select();
      return _this2.success('success');
    })();
  }

  signVerifyAction() {
    var _this3 = this;

    return _asyncToGenerator(function* () {
      const sign = _this3.post('sign');
      const tradeNo = _this3.post('tradeNo');
      const appId = _this3.post('appId');
      const authAppId = _this3.post('authAppId');
      const outTradeNo = _this3.post('outTradeNo');
      const sellerId = _this3.post('sellerId');
      const timestamp = _this3.post('timestamp');
      const totalAmount = _this3.post('totalAmount');

      // var signature = decodeURIComponent(sign,'base64')
      // var verifier = crypto.createSign('RSA-SHA256');
      // const private_key = fs.readFileSync('./app_private_key.pem') 
      // var info = 'app_id='+appId+'&auth_app_id'+authAppId+'&charset=utf-8&method=alipay.trade.page.pay.return&out_trade_no='+outTradeNo+'&seller_id='+sellerId+'&timestamp='+timestamp+'&total_amount='+totalAmount+'&trade_no='+tradeNo+'&version=1.0';
      // verifier.update(info) 
      // var signOrder = encodeURIComponent(verifier.sign(private_key, 'base64'))
      // let result = verifier.verify('-----BEGIN PUBLIC KEY----------END PUBLIC KEY-----', signature, 'base64');
      //return this.success({result:result,userToken:this.ctx.state.token})
      return _this3.success(_this3.ctx.state.token);
    })();
  }
};
//# sourceMappingURL=alipay.js.map
...
module.exports = class extends Base {
  async indexAction() {
    ...
    const hotGoods = await this.model('goods').field(['id', 'name', 'list_pic_url', 'retail_price', 'goods_brief']).where({is_hot: 1}).limit(4).select();
    ...
    for (const categoryItem of categoryList) {
      ...
      if(think.isEmpty(childCategoryIds))
        continue

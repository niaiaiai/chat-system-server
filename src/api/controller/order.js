...
module.exports = class extends Base {
  ...
  async submitOrderAction() {
    ...
    const district = this.post('district')
  ...
  async getOrdersAction() {
    ...
    const userId = this.post('userId')
    const orderStatus = this.post('orderStatus')
    ...
    let order = []
    if(orderStatus == 8) {
      count = this.model('order').where({user_id:userId}).select()
      orderData = await this.model('order').where({user_id:userId}).page(page,pageSize).select()// join({
      for(var i=0; i<orderData.length; i++) {
        ...
        orderData[i] = await this.model('order_goods').where('nideshop_order_goods.order_id = '+ orderData[i].order_sn).join({
          order_express: {
            join: 'left',
      ...
      count = this.model('order').where({user_id:userId,order_status:orderStatus}).select()
    ...
    return this.success({detail:orderData,order:order,count:count.length})
  }
};
